...
useEffect(() => {
  ...
  const mergedCategories = [
    ...new Set([...defaultCategories, ...savedCat]),
  ];
    ...
    <div className="flex items-center justify-between px-8 py-4 bg-white text-black shadow sticky top-0 z-50">
      ...
      <div className="flex items-center gap-8">
        ...
        <div className="flex gap-6 text-sm">
          <span onClick={() => router.push("/user/StorePage")} className="cursor-pointer hover:text-orange-500">
            Store
          </span>

          <span onClick={() => router.push("/support")} className="cursor-pointer hover:text-orange-500">
            Support
          </span>
        </div>
          ...
          <div key={i} className="mb-6">
            ...
            {selectedCategory === cat && (
              <div className="ml-3 mt-2 space-y-2">
                {(subcategories[cat.toLowerCase()] || []).map((sub, j) => (
                  <p
                    key={j}
              ...
              {results.length === 0 ? (
                ...
                results.map((item, i) => (
                  <div
                    key={i}
                   onClick={() =>
  router.push(`/user/ProductDetails/${encodeURIComponent(item.name)}`)
}
  ...
  className="cursor-pointer hover:text-orange-500"
          ...
          {openUser && (
            <div className="absolute right-0 mt-3 w-44 bg-white shadow-xl rounded-xl p-3 border">
              {!user || !user.isLogin ? (
                <>
                  <button onClick={() => router.push("/login")} className="block w-full text-left px-3 py-2 hover:bg-gray-100 rounded">
                    Login
                  </button>
                  <button onClick={() => router.push("/signup")} className="block w-full text-left px-3 py-2 hover:bg-gray-100 rounded">
                    Register
                  </button>
                </>
              ) : (
                <>
                  ...
                  <button onClick={() => router.push("/support/MyAccount")} className="block w-full text-left px-3 py-2 hover:bg-gray-100 rounded">
                    My Profile
                  </button>

                  <button onClick={() => router.push("/user/OrdersPage")} className="block w-full text-left px-3 py-2 hover:bg-gray-100 rounded">
                    My Orders
                  </button>
